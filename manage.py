    ...
    for l in links:
        if continuar:
            ...
            if len(l['imdb']) > 0 and (db.registros.find({'magnet':l['link']}).count()==0 or forcar) and db.legendado.find({'magnet':ha}).count()==0:
                ...
                #socketio.emit('atualizar','Carregando: https://www.skytorrents.lol/torrent/' + ha)
                #r = session.get('https://www.skytorrents.lol/torrent/' + ha)
                ...
                #texto_completo = r.html.find(                '.column.is-8.has-text-centered > div > div > table > tbody',                first=True)
                #tipo = 'sky'
                
				#if texto_completo != None:
                #    if "DUAL" in texto_completo.text or "DUBLADO" in texto_completo.text:
                #        ingles = None
                #else:
                ...
                if ingles == None:
                    ...
                    for a in arquivos:               
                        ...
                        try:
                            if (texto[-3:] not in [
                                'exe', 'txt', 'url', 'srt', 'peg', 'jpg','png',
                                'nfo', 'zip'
                        ]) and (texto[-10:] not in [
                                'sample.mkv'
                        ]) and (ar not in [
                                "COMANDOTORRENTS.COM.mp4", "BLUDV.TV.mp4", "BLUDV.mp4","LAPUMiA.mp4",
                                "File Name"
                        ]) and ("LEGENDADO" not in texto):
                    ...
                    socketio.emit(
                    'atualizar', 'Legendado ' + titulo +
                    ': https://btdb.eu/torrent/' + ha)
            else:
...
def limpar():
    for r in db.registros.find({
        ...
        [{
            "nome": {
                "$regex":
                "\.srt|\.url|\.exe|\.sub|\.txt|\.nfo|\.jpeg|\.jpg|\.png|\.zip|sample"
            }
        }, {
            "nome": "BLUDV.TV.mp4"
        }, {
...
def thread_link(q, im):
    ...
    r2 = session.get(l)
    try:
        id_imdb = r2.html.find(
            "a[href*='www.imdb.com']", first=True).attrs['href']
        id_imdb = id_imdb.replace("http://www.imdb.com/title/", "").replace(
            "https://www.imdb.com/title/", "").replace("/", "").replace(
                "/", "").replace("?ref_=nv_sr_", "").replace(
                    "?ref_=plg_rt_1", "").replace("http:www.imdb.com",
                                                  "").strip()
    except:
        print('Erro IMDB')
    ...
    for html in r2.html.find(
            'a[href^="http://www.meucarrao.info/magnet/?xt"]'):
        s.append({'imdb': id_imdb, 'link': list(html.links)[0]})
...
def thread_lista(url, tamanho):
    ...
    for x in range(1, tamanho):
        ...
        for elem in titulos:
            ...
            dublado = elem.find(
                '.idioma_lista', first=True).text.strip()
            # print(link, dublado)
            if dublado == "Dublado":
                print(link)
                ...
                try:
                    id_imdb = r2.html.find(
                        "a[href*='www.imdb.com']", first=True).attrs['href']
                    id_imdb = id_imdb.replace(
                        "http://www.imdb.com/title/", "").replace(
                            "https://www.imdb.com/title/", "").replace(
                                "/", "").replace("/", "").replace(
                                    "?ref_=nv_sr_", "").replace(
                                        "?ref_=plg_rt_1", "").replace(
                                            "http:www.imdb.com", "").strip()
                except:
                    id_imdb = ""
...
def thread_lista_pirate(url, tamanho):
    s = []
    print(url)
    r = session.get(url)
    titulos = r.html.find('#content article')
    for elem in titulos:
        # print(elem.find('a',first=True).attrs)
        link = elem.find(
            'a', first=True).attrs['href']
